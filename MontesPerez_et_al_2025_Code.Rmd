...
This is the code used to generated results and plots presented in **Naturalization of Artificial Urban Ponds Alters Greenhouse Gas Dynamics - Montes-Pérez et al., 2025** research article.
...
#Download Ponds_GHG_EnvironmentalVars-----
  inUrl1  <- "https://pasta.lternet.edu/package/data/eml/edi/1927/1/3e128569a1bbc4f38cc88bd46e8a310b" 
  infile1 <- tempfile()
  ...
  if (class(dt1$Naturalization)!="factor") dt1$Naturalization<- as.factor(dt1$Naturalization)
...
#Download CO2eq----
  inUrl2  <- "https://pasta.lternet.edu/package/data/eml/edi/1927/1/1588bee09b6afab3fcd7670f839659e9" 
  infile2 <- tempfile()
  ...
  if (class(dt2$Naturalization)!="factor") dt2$Naturalization<- as.factor(dt2$Naturalization)                                   
...
```{r}
  ...
  data_l <- data_l %>% group_by(Naturalization, variable, Pond_name) %>% 
    ...
    ungroup()
  add_total_flux <- data_l %>% group_by(Naturalization, Pond_name) %>% summarise(Flux_CO2eq = sum(Flux_CO2eq)) %>% 
  ungroup()  %>% 
  mutate(variable = "Total")

data_l %>% bind_rows(add_total_flux) %>% 
  ggplot(aes(x = Naturalization, y = Flux_CO2eq, fill = variable))+
  geom_boxplot()+
